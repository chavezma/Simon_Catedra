...
int main(void)
{
  ...
  */
  ...
  Teclado ();
  //ConversorAD ();
  if(Switch_Vd_On) {Led_Vd_On;} else {Led_Vd_Off;}  	// Código para probar switches
  if(Switch_Rj_On) {Led_Rj_On;} else {Led_Rj_Off;}
  if(Switch_Am_On) {Led_Am_On;} else {Led_Am_Off;}
  if(Switch_Az_On) {Led_Az_On;} else {Led_Az_Off;}

  switch (Estado) {
	case 0: {											//Estado permanente
			char cambiaseg = segundo;
			get_time();
			if(cambiaseg != segundo){Display_DiaHora();}
			if(Tecla==12){Estado = 2;}					// Tecla B presionada
			if(Tecla==11){Estado = 1;}					// Tecla A presionada
			Tecla = 0;
			}	break;
		...
		case 1: {										//Estado 1
			lcd_put_cur(0, 0);
			lcd_send_cmd(0x0C);  						// Cursor Off  Blink Off
			lcd_send_string("Estado 1        ");
			lcd_put_cur(1, 0);
			lcd_send_string("                ");
			if(Tecla==15){Estado = 0;lcd_clear ();}
			Tecla = 0;
			}	break;
		...
		case 2: {										//Estado programacion año decena
			 ...
			 Estado = 0;
			 Tecla = 0;
			 lcd_put_cur(0, 0);  						// Position
			 lcd_send_string("Estado 2        ");
			 lcd_put_cur(1, 0);
			 lcd_send_string("                ");
			 lcd_put_cur(1, 0);
			 lcd_send_cmd(0x0D);  						// Cursor On   Blink On
			 while(count1){
				 Teclado ();
				 if(Tecla){count1 = 10;}				// Mantiene Loop interno con cualquier
				 										// tecla del pad presionada
				 if(Tecla==16){	 						// Tecla #
					 lcd_put_cur(0, 0);  				// Position
					 lcd_send_string("Loop Interno    ");}
				 if(Tecla==15){count1 = 1;Enter_Esc_flag = 0;Estado = 0;} //  Tecla *  Escapa
				 HAL_Delay(100);
			 Tecla = 0;
			 count1--;									// Escapa del Loop cuando count1 llega a 0
			 }
			 lcd_send_cmd(0x0C);  						// Cursor Off  Blink Off
			 }	break;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0|GPIO_PIN_10|GPIO_PIN_15|GPIO_PIN_3|GPIO_PIN_4
                          |GPIO_PIN_5|GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_13, GPIO_PIN_RESET);
  ...
  GPIO_InitStruct.Pin = GPIO_PIN_0|GPIO_PIN_10|GPIO_PIN_15|GPIO_PIN_3|GPIO_PIN_4
                          |GPIO_PIN_5|GPIO_PIN_8|GPIO_PIN_9|GPIO_PIN_2|GPIO_PIN_13;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
{
	Tecla = 0;
	Row1_On;Row2_Off;Row3_Off;Row4_Off;
	HAL_Delay(20);
	if(Column1_On){Tecla = 1;}
	if(Column2_On){Tecla = 2;}
	if(Column3_On){Tecla = 3;}
	if(Column4_On){Tecla = 11;}
	Row1_Off;Row2_On;Row3_Off;Row4_Off;
	HAL_Delay(20);
	if(Column1_On){Tecla = 4;}
	if(Column2_On){Tecla = 5;}
	if(Column3_On){Tecla = 6;}
	if(Column4_On){Tecla = 12;}
	Row1_Off;Row2_Off;Row3_On;Row4_Off;
	HAL_Delay(20);
	if(Column1_On){Tecla = 7;}
	if(Column2_On){Tecla = 8;}
	if(Column3_On){Tecla = 9;}
	if(Column4_On){Tecla = 13;}
	Row1_Off;Row2_Off;Row3_Off;Row4_On;
	HAL_Delay(20);
	if(Column1_On){Tecla = 15;}
	if(Column2_On){Tecla = 10;}
	if(Column3_On){Tecla = 16;}
	if(Column4_On){Tecla = 14;}
	Row1_Off;Row2_Off;Row3_Off;Row4_Off;
}

void Validador_dia (void)
{
	if((anio==24||anio==28||anio==32||anio==36||anio==40||anio==44||anio==48||anio==52||anio==56)&&mes==2)
		{
		Limite_dia_decena = 3;
		Limite_dia_unidad = 10;
		}
	else if(mes==2)	{Limite_dia_decena = 3;Limite_dia_unidad = 9;}
	if(mes==4||mes==6||mes==9||mes==11)
		{
		Limite_dia_decena = 4;
		if(dia>29){Limite_dia_unidad = 1;}
		else Limite_dia_unidad = 10;
		}
	if(mes==1||mes==3||mes==5||mes==7||mes==8||mes==10||mes==12)
		{
		Limite_dia_decena = 4;
		if(dia>29){Limite_dia_unidad = 2;}
		else Limite_dia_unidad = 10;
		}
}
